...
export default function Appointments() {
    ...
    // Consultation type — "onsite" | "online"  (your feature)
    ...
    const fetchUnavailableSlots = async () => {
        try {
            ...
            // silent fail — calendar shows all slots as available
        }
    };
    ...
    const submitForm = async (values, authToken) => {
        const {
            ...
            captchaToken: ct, consultationType: ctype,
        } = values;
        ...
        try {
            ...
            const res = await fetch(`${API_BASE}/api/consultations`, {
                ...
                body: JSON.stringify({
                    ...
                    consultation_type: ctype ?? "onsite",   // your feature
                }),
            });
    ...
    const handleAppointmentSubmit = async (e) => {
        ...
        // Groupmate's slot double-check at submit time
        if (appointmentDate && appointmentTime && isTimeSlotUnavailable(appointmentDate, appointmentTime)) {
            newErrors.appointmentTime = "This time slot is no longer available. Please choose another.";
        }
        ...
        if (!token) {
            sessionStorage.setItem(DRAFT_KEY, JSON.stringify({
                ...
                projectType, appointmentMessage, appointmentDate,
                appointmentTime, captchaToken,
                consultationType,   // your feature
            }));
    ...
    // ─── Slot availability helpers (groupmate's logic, kept intact) ──────────
    const isDateFullyBooked = (date) => {
        const slotsForDate = unavailableSlots.filter((slot) => {
            const slotDate = slot.blocked_date?.split("T")[0] || slot.blocked_date;
    ...
    const isTimeSlotUnavailable = (date, time) => {
        ...
        const dateSlots = unavailableSlots.filter((slot) => {
            const slotDate = slot.blocked_date?.split("T")[0] || slot.blocked_date;
            return slotDate === date;
        });

        const exactMatch = dateSlots.some((slot) => slot.blocked_time === time);
        ...
        if (bookedOnly.length === 0) return false;

        const latestBooking  = bookedOnly[bookedOnly.length - 1];
        const latest         = new Date(`${date}T${latestBooking.blocked_time}:00`);
        const nextAvailable  = new Date(latest.getTime() + 2 * 60 * 60 * 1000);
        const requested      = new Date(`${date}T${time}:00`);

        return requested < nextAvailable;
    };

    // ─── Loading / active-consult guards ─────────────────────────────────────
    if (checkingActive) {
        return (
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-16 lg:items-start">
                    <div className="lg:col-span-7">
                        <form onSubmit={handleAppointmentSubmit} className="w-full">
                            ...
                            {/* ── Section 03: Consultation Format (your feature) ── */}
                            <div className="mb-20">
                                <div className="border-b-2 border-black pb-4 mb-10 flex justify-between items-end">
                                    <h2 className="text-xl md:text-2xl font-bold tracking-tight uppercase">Consultation Format</h2>
                                    <span className="text-xs font-bold tracking-widest text-neutral-400">03</span>
                                </div>
                                <ConsultationTypeToggle value={consultationType} onChange={setConsultationType} />
                            </div>
                            ...
                            <div className="mb-10">
                                <div className="border-b-2 border-black pb-4 mb-10 flex justify-between items-end">
                                    ...
                                    <span className="text-xs font-bold tracking-widest text-neutral-400">04</span>
                                </div>
                                <div className="w-full">
                                    <label className={`block text-[11px] font-bold tracking-[0.15em] uppercase mb-2 transition-colors ${errors.appointmentDate || errors.appointmentTime ? "text-red-500" : "text-neutral-800"}`}>
                                        Select Date & Time *
                                    </label>
                            ...
                            <div className="mb-10">
                                {RECAPTCHA_SITE_KEY ? (
                                    <ReCAPTCHA
                                        ...
                                        onChange={(token) => { setCaptchaToken(token); setErrors((prev) => ({ ...prev, captcha: undefined })); }}
                                        onExpired={() => setCaptchaToken(null)}
                                        onErrored={() => { setCaptchaToken(null); setErrors((prev) => ({ ...prev, captcha: "Captcha failed to load. Please check your reCAPTCHA key and domain settings." })); }}
                                    />
                                ) : (
                                    <div className="border border-red-500 bg-red-50 px-4 py-3">
                                        <p className="text-[11px] tracking-wide text-red-500 uppercase font-bold">Missing VITE_RECAPTCHA_SITE_KEY in .env file.</p>
                                    </div>
                                )}
...
/* ──────────────────────────────────────────────────────────────────────────── */
/*  Consultation Type Toggle  (your feature)                                     */
/* ──────────────────────────────────────────────────────────────────────────── */
    ...
    return (
        <div className="space-y-6">
            <div className="grid grid-cols-2 gap-4">
                {[
                    {
                        id: "onsite", label: "Onsite Visit", sub: "Meet at our studio",
                        icon: (
                    ...
                    {
                        id: "online", label: "Online / Video", sub: "Join via video call",
                        icon: (
                            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" className="w-5 h-5">
                    ...
                    return (
                        <button key={opt.id} type="button" onClick={() => onChange(opt.id)}
                            className={`group flex flex-col items-start gap-3 p-5 border-2 transition-all text-left cursor-pointer ${active ? "border-black bg-black text-white" : "border-neutral-200 bg-white text-black hover:border-neutral-400"}`}>
                            <span className={`transition-colors ${active ? "text-white" : "text-neutral-500 group-hover:text-black"}`}>{opt.icon}</span>
                            <span className="text-[11px] font-bold tracking-[0.15em] uppercase leading-none">{opt.label}</span>
                            <span className="text-[11px] tracking-wide leading-none text-neutral-400">{opt.sub}</span>
                        </button>
                    );
                })}
            </div>
            ...
            <AnimatePresence mode="wait">
                {value === "online" && (
                    <motion.div key="online-notice" initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} transition={{ duration: 0.25 }}
                        className="border-l-2 border-blue-500 bg-blue-50 pl-4 py-3 pr-4">
                        <p className="text-[11px] font-bold tracking-[0.15em] uppercase text-blue-700 mb-1">Video Call</p>
                ...
                {value === "onsite" && (
                    <motion.div key="onsite-notice" initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} transition={{ duration: 0.25 }}
                        className="border-l-2 border-black bg-neutral-50 pl-4 py-3 pr-4">
                        <p className="text-[11px] font-bold tracking-[0.15em] uppercase text-neutral-700 mb-1">Studio Address</p>
...
function OngoingConsultationBlock({ consultation, onDashboard }) {
    ...
    const formattedDate = consultation?.consultation_date
        ? new Date(String(consultation.consultation_date).replace(" ", "T")).toLocaleDateString("en-US", { weekday: "long", year: "numeric", month: "long", day: "numeric" })
        : "To be confirmed";

    const formattedTime = consultation?.consultation_date
        ? new Date(String(consultation.consultation_date).replace(" ", "T")).toLocaleTimeString("en-US", { hour: "numeric", minute: "2-digit", hour12: true })
        : "";

    const isOnline = String(consultation?.consultation_type ?? "onsite").toLowerCase() === "online";

    const rows = [
        ...(consultation?.reference_id ? [{ label: "Reference No.", value: consultation.reference_id, mono: true }] : []),
                    ...
                    <div className="lg:col-span-7">
                        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }} className="mb-12">
                            <div className="border-b-2 border-black pb-4 mb-10 flex justify-between items-end">
                                ...
                                <span className={`text-[10px] font-bold tracking-widest uppercase px-3 py-1 ${s.bg} ${s.text} border ${s.border}`}>{s.label}</span>
                            </div>

                            <p className="text-[14px] leading-relaxed text-neutral-600 mb-10">
                                You currently have an ongoing consultation request. A new appointment cannot be scheduled until your current one is resolved.
                                Visit your dashboard to view details, track its status, or contact us for assistance.
                            </p>
                                ...
                                <div className="divide-y divide-neutral-100">
                                    {rows.map(({ label, value, highlight, color, mono }) => (
                                        <div key={label} className="flex justify-between items-center px-6 py-4">
                                            ...
                                            <span className={`text-[13px] font-semibold ${highlight ? color : "text-neutral-800"} ${mono ? "font-mono tracking-wider" : ""}`}>{value}</span>
                                        </div>
                                    ))}
                                </div>
                            </div>

                            {/* Zoom link shown when online consultation */}
                            {isOnline && consultation?.zoom_link && (
                                <div className="mb-10 border-l-2 border-blue-500 bg-blue-50 pl-4 py-3 pr-4">
                                    <p className="text-[11px] font-bold tracking-[0.15em] uppercase text-blue-700 mb-2">Your Video Call Link</p>
                                    <a href={consultation.zoom_link} target="_blank" rel="noopener noreferrer"
                            ...
                            <div className="flex flex-col sm:flex-row gap-4">
                                <button onClick={onDashboard} className="bg-black text-white px-10 py-4 text-[11px] font-bold tracking-[0.2em] uppercase hover:bg-neutral-800 cursor-pointer">
                                    Go to Dashboard
                                </button>
                                <a href="mailto:hello@rmty.com" className="border border-black px-10 py-4 text-[11px] font-bold tracking-[0.2em] uppercase hover:bg-black hover:text-white text-center cursor-pointer">
                                    Contact Us
                                </a>
                            </div>
                        </motion.div>
...
function UnderlineInput({ label, type = "text", options, isPhone, placeholder, value, onValueChange, externalError }) {
    ...
    const inputClass = `w-full bg-transparent border-b px-0 py-2 text-sm outline-none transition-colors rounded-none appearance-none ${hasError ? "border-red-500 text-red-500" : "border-neutral-300 focus:border-black text-black"}`;
